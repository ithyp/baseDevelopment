...
class  VisionDom extends VisionBase  {
    static createElement(tag,attrs,...children){
     ...
     getRealDom(virtualObject,realView){
        ...
        if(this.isObject(vo)){
            ...
            if (realView) {
                ...
                if (type===3) {
                    if ( realView.textContent !=  virtualObject) {
        ...
        let oChild,
            ...
            if(nChildLen){
                  for(let k  in nChild){
                      ...
                      if(!child){         
                          for(let i = min;  i < oChildLen; ++i){
                               let oc =   oChild[i]
                                ...
                                if(oc && this.isSameNodeType( oc ,nChildDom )){
                                    child = oc
                                    oChild[i] = null;
                                    if (i===oChildLen-1) oChildLen--;
                                    if (i===min+1) min++;
                                    break;
                                }
                          }
                      ...
                      child = this.getRealDom( nChildDom,child);
